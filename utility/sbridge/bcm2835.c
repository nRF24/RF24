...
#define CMD_gpio_write      0x01
#define CMD_gpio_fsel       0x02
#define CMD_spi_begin       0x03
#define CMD_spi_transfer    0x04
#define CMD_spi_transfernb  0x05
#define CMD_spi_chipSelect  0x06
...
uint8_t bcm2835_spi_transfer(uint8_t value)
{
    ...
    if(ret[1] != 0xAF)
    {
      printf("spi_transfer - comm error!\n");
      exit(-1);
    }
    
    bcm2835_spi_chipSelect(1);//liga CSN
...
void bcm2835_spi_transfernb(char* tbuf, char* rbuf, uint32_t len)
{
    unsigned char buff[2];
    unsigned char ret;
    buff[0]=CMD_spi_transfernb;
    buff[1]=len;
    int i;

    if(len >255)
    {
      printf("transfernb overflow!\n");
      exit(-1);
    }
    ...
    if(ret != 0xAF)
    {
      printf("spi_transfernb - comm error!\n");
      exit(-1);
    }
...
void bcm2835_spi_chipSelect(uint8_t cs)
{
    ...
    if(ret != 0xAF)
    {
      printf("gpio_chipSelect - comm error!\n");
      exit(-1);
    }
...
int bcm2835_init(void)
{
        ...
        if (sfd <0) {perror(MODEMDEVICE); exit(-1); }
        ...
        newtio.c_cc[VTIME]    = 2;   /* inter-character timer 0.2s */
