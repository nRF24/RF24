  ...
  if (role) {
    ...
    for (int i = 0; i < 2; i++) // iterate twice to transmit, because we split our payload into two pieces
    {
      ...
      if (report) {
        ...
        int number_of_payload_elements = sizeof(split_payload_2d_array[i]) / sizeof(split_payload_2d_array[0][0]);  // get number of elements by dividing the size of split_payload_2d_array by its first element
